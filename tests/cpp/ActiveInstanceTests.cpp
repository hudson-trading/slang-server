...
TEST_CASE("EditingActiveInstanceRebindsTypeParameterToShallowCompilation") {
    std::string instancePath;
    SECTION("Direct package type") {
        instancePath = "top.selected_leaf";
    }
    SECTION("Local alias of package type") {
        instancePath = "top.aliased_leaf";
    }
    SECTION("Chain of local aliases") {
        instancePath = "top.chained_leaf";
    }
    SECTION("Alias forwarded through a type parameter") {
        instancePath = "top.wrapper.inst";
    }

    ServerHarness server("active_instance_type_parameter");
    server.setBuildFile("design.f");

    auto doc = server.openFile("leaf.sv");
    REQUIRE(doc.getDiagnostics().empty());
    REQUIRE(server.setActiveInstance(instancePath));
...
TEST_CASE("EditingActiveInstancePreservesParentLocalType") {
    ServerHarness server("active_instance_type_parameter");
    server.setBuildFile("design.f");

    auto doc = server.openFile("passthrough.sv");
    REQUIRE(server.setActiveInstance("top.local_leaf"));
    auto* instance = server.m_driver->comp->getActiveInstanceSymbol("passthrough");
    REQUIRE(instance);
    auto* designType = instance->body.lookupName("T");
    REQUIRE(designType);
    ...
    CHECK(doc.getDiagnostics().empty());
    auto* shallowType = doc.doc->getAnalysis()->getCompilation()->getRoot().lookupName(
        "passthrough.T");
    REQUIRE(shallowType);
    CHECK(shallowType->as<slang::ast::Type>().isEnum());
    CHECK(&shallowType->as<slang::ast::Type>().getCanonicalType() ==
          &designType->as<slang::ast::Type>().getCanonicalType());
}
