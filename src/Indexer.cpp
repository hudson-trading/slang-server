    ...
    if (indexConfigs.empty()) {
        ...
        for (const auto& cfg : indexConfigs) {
            for (const auto& dir : cfg.dirs) {
                ...
                collectFilesFromDirectory(fullDirPath,
                                          cfg.excludeDirs.value_or(std::vector<std::string>{}),
                                          pathsToIndex);
            }
        }
    }
...
void Indexer::indexAndReport(std::vector<fs::path> pathsToIndex, uint32_t numThreads) {
    ...
    INFO("Indexing complete: {} symbols (~{} KB), {} macros (~{} KB), {} unique URIs (~{} KB)",
         symbolToFiles.size(), symbolsSize / 1024, macroToFiles.size(), macrosSize / 1024,
         uniqueUris.size(), urisSize / 1024);
