    ...
    for (auto* parameterBase : designBody.getParameters()) {
        ...
        if (auto* parameter = symbol.as_if<ast::ParameterSymbol>()) {
            ...
            if (!designType.isError()) {
                ...
                if (!designType.name.empty()) {
                    // Named types are identity-based, so resolve package types in the shallow
                    // compilation instead of retaining a pointer into the full design. Looking up
                    // through the root would finalize top instances before overrides are complete.
                    auto path = designType.getLexicalPath();
