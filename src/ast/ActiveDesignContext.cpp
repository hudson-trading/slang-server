...
public:
    ...
    InterfacePortSyntaxIndex(const ast::DefinitionSymbol& definition,
        ...
        std::unordered_map<std::string_view, const syntax::DeclaratorSyntax*> declarations;
    ...
    const syntax::DeclaratorSyntax* get(std::string_view name) const {
        auto it = m_ports.find(name);
        return it == m_ports.end() ? nullptr : it->second;
    }

private:
    std::unordered_map<std::string_view, const syntax::DeclaratorSyntax*> m_ports;
};
...
void copyParameterOverrides(ast::HierarchyOverrideNode& shallowOverride,
    ...
    for (auto* parameterBase : designBody.getParameters()) {
        ...
        if (auto* parameter = symbol.as_if<ast::ParameterSymbol>()) {
            ...
            const auto& designType = parameter->targetType.getType();
            if (!designType.isError()) {
                const ast::Type* shallowType = nullptr;
                for (auto* type = &designType; type && !shallowType;) {
                    // Named types are identity-based, so resolve package types in the shallow
                    // compilation, following local typedefs and forwarded type parameters to their
                    // targets. Looking up through the root would finalize top instances before
                    // overrides are complete.
                    auto path = type->name.empty() ? std::string{} : type->getLexicalPath();
                    auto separator = path.find("::");
                    if (separator != std::string::npos) {
                        auto* package = shallowCompilation.getPackage(
                            std::string_view(path).substr(0, separator));
                        auto* shallowSymbol =
                            package
                                ? package->lookupName(std::string_view(path).substr(separator + 2),
                                                      ast::LookupLocation::max)
                                : nullptr;
                        shallowType = shallowSymbol ? shallowSymbol->as_if<ast::Type>() : nullptr;
                    }
                    auto* alias = type->as_if<ast::TypeAliasType>();
                    type = alias ? &alias->targetType.getType() : nullptr;
                }

                shallowOverride.paramOverrides.insert_or_assign(
                    syntax, ast::HierarchyOverrideNode::TypeParamOverride{
                                shallowType ? shallowType : &designType,
                                parameter->targetType.getResolvedTypeSyntax()});
            }
        }
    }
}

const ast::Symbol* lookupDesignSymbol(std::string_view path, ast::Compilation& designCompilation) {
    ast::LookupResult result;
    ast::ASTContext context(designCompilation.getRoot(), ast::LookupLocation::max);
    ast::Lookup::name(designCompilation.parseName(path), context,
                      ast::LookupFlags::AllowUnnamedGenerate, result);
    return result.found;
}

const ast::Symbol* mapDesignSymbol(const ast::Symbol& shallowSymbol,
                                   const ast::InstanceBodySymbol& shallowBody,
                                   const ast::InstanceSymbol& designInstance,
                                   ast::Compilation& designCompilation) {
    if (&shallowSymbol == &shallowBody)
        return &designInstance.body;

    auto shallowInstancePath = shallowBody.parentInstance->getHierarchicalPath();
    auto symbolPath = shallowSymbol.getHierarchicalPath();
    if (!std::string_view(symbolPath).starts_with(shallowInstancePath))
        return nullptr;

    auto designPath = designInstance.getHierarchicalPath();
    designPath.append(std::string_view(symbolPath).substr(shallowInstancePath.size()));
    return lookupDesignSymbol(designPath, designCompilation);
}
...
void ActiveDesignContext::bindInstance(const ast::InstanceSymbol& instance) {
    ...
    for (auto* symbol : instance.body.getPortList()) {
        if (auto* port = symbol->as_if<ast::InterfacePortSymbol>()) {
            ...
            if (auto* designInstance = getFirstInterfaceInstance(connection.resolvedEndpoint)) {
                m_interfaceInstances.push_back(
                    {.portLocation = port->location, .designInstance = designInstance});
            }
            binding.interfaceConnections.emplace(std::string(port->name), std::move(connection));
        }
    }
...
const ast::Symbol* ActiveDesignContext::getDesignSymbol(const ast::Symbol& shallowSymbol) const {
    ...
    auto& designCompilation = m_analysis->compilation;
    auto symbolPath = shallowSymbol.getHierarchicalPath();
    if (!symbolPath.empty() && symbolPath.front() != '.') {
        if (auto* symbol = lookupDesignSymbol(symbolPath, designCompilation))
            return symbol;
    }

    auto interfaceInstance = std::ranges::find(m_interfaceInstances, body->parentInstance->location,
                                               &InterfaceInstanceBinding::portLocation);
    if (interfaceInstance != m_interfaceInstances.end()) {
        return mapDesignSymbol(shallowSymbol, *body, *interfaceInstance->designInstance,
                               designCompilation);
    }
    ...
    if (binding == m_definitions.end())
        return nullptr;
    return mapDesignSymbol(shallowSymbol, *body, *binding->second.instance, designCompilation);
}
