...
void setActiveInstanceChain(
    ...
    auto* current = &target;
    while (current) {
        activeInstancesByModule[std::string(current->getDefinition().name)] =
            hier::toQualifiedInstance(*current, sourceManager);
        setConnectedInterfaceInstances(*current, activeInstancesByModule, sourceManager);
...
bool isWithinHierarchyPath(std::string_view path, std::string_view parent) {
    if (!path.starts_with(parent))
        return false;
    return path.size() == parent.size() || path[parent.size()] == '.' || path[parent.size()] == '[';
}

size_t hierarchyDepth(std::string_view path) {
    return 1 + std::count_if(path.begin(), path.end(), [](char c) { return c == '.' || c == '['; });
}
...
// Re-resolve a full-comp symbol against the doc that owns its source so we get a
// fresh location after edits. Previously this iterated every document and triggered a
// shallow-analysis build on each, which was cripplingly slow on large designs (one
// `setActiveInstance` triggered ~140 file analyses). Find the owning doc by source
// path and only rebuild that one.
...
std::optional<std::string> ServerCompilation::getActiveInstanceAtPosition(
    std::string_view moduleName, const lsp::TextDocumentPositionParams& params) {
    auto activeInstance = getActiveInstance(std::string(moduleName));
    if (!activeInstance)
        return std::nullopt;
    ...
    for (auto& candidate : matchingPaths) {
        if (!isWithinHierarchyPath(candidate, activeInstance->instPath))
            continue;

        size_t conflicts = 0;
        int matches = 0;
        for (const auto& [arrayPath, selectedPath] : m_activeGenerateScopesByArray) {
            if (!candidate.starts_with(arrayPath) || candidate.size() <= arrayPath.size() ||
                candidate[arrayPath.size()] != '[') {
                continue;
            }
            if (isWithinHierarchyPath(candidate, selectedPath))
                matches++;
            else
                conflicts++;
        }

        auto score = std::tuple(conflicts, -matches, -static_cast<int>(hierarchyDepth(candidate)),
                                candidate);
    ...
    return bestPath.value_or(activeInstance->instPath);
}
