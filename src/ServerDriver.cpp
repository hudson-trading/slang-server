...
void ServerDriver::parseAndLoadSources(const std::vector<std::string>& buildfiles) {
    ...
    bool ok = true;
    for (auto& [filePath, flags] : m_config.flagsByFile.value()) {
        driver.currentParseSource = filePath;
        ok &= driver.parseCommandLine(flags, parseOpts);
    }
...
std::vector<lsp::LocationLink> ServerDriver::getDocDefinition(const URI& uri,
    ...
    if (pathStr.empty() || pathStr[0] == '<') {
        ...
        if (!OS::readFile(srcPath, buf)) {
            ...
            std::string patterns[] = {"-D" + macroName, "+define+" + macroName};
