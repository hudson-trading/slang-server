...
InlayHintCollector::InlayHintCollector(const ShallowAnalysis& analysis, lsp::Range range,
    ...
    m_analysis(analysis), m_range(range), m_config(config) {
}
...
void InlayHintCollector::handle(const HierarchyInstantiationSyntax& syntax) {
    ...
    // Do params
    if (m_config.orderedInstanceNames.value() && syntax.parameters) {
        size_t paramIndex = 0;
        ...
        for (auto portSyntax : hierInstSyntax.connections) {
            switch (portSyntax->kind) {
                case SyntaxKind::OrderedPortConnection: {
                    if (!m_config.orderedInstanceNames.value()) {
                        continue;
                    }
                    result.push_back(lsp::InlayHint{
                ...
                case SyntaxKind::WildcardPortConnection: {
                    if (!m_config.wildcardNames.value()) {
                        continue;
                    }
                    std::string label;
...
void InlayHintCollector::handle(const MacroUsageSyntax& syntax) {
    if (!m_config.macroArgNames.value() || !syntax.args) {
        return;
    }
    ...
    // Not worth showing names for single arg macros
    if (syntax.args->args.size() <= 1) {
        return;
    }
...
void InlayHintCollector::handle(const InvocationExpressionSyntax& syntax) {
    if (!m_config.funcArgNames.value() || !syntax.arguments)
        return;
    ...
    // Not worth showing names for single arg functions
    if (syntax.arguments->parameters.size() <= 1) {
        return;
    }
