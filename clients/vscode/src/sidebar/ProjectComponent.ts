    ...
    implements TreeDataProvider<HierItem>, vscode.TerminalLinkProvider<InstanceLink>
{
            ...
            if (instance === undefined) {
                if (this.unit === undefined) {
                    ...
                    await vscode.window.showInformationMessage(
                        'Please set top level or build file first'
                    )
                }
    ...
    async setTopModule() {
        ...
        const tops = this.unit.children.filter(
            (item) => item.inst.kind === slang.SlangKind.Instance
        )
    ...
    async resolveTreeItem(
        ...
        item.command = {
            ...
            arguments: [
                element,
                { revealHierarchy: false, revealFile: true, revealInstance: true },
            ],
        }
